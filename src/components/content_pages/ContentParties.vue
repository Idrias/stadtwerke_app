        ...
        <div id="partyCreator" class="shadow">
          ...
          <table v-if="parties.length > 0">
            ...
            <tr
              ...
              v-on:click="$refs.addParty.openForChange(party)"
            >
              <td>{{ party.name }}</td>
            </tr>
          </table>
              ...
              <table>
                ...
                >
                  ...
                  <td>
                    <input
                      type="number"
                      v-model.number="meter.shares[party.uuid]"
                      min="0"
                    />
                  </td>
                  <td>({{ getPercentage(meter, party) }}%)</td>
                </tr>
              </table>
  ...
  methods: {
    getPercentage(meter, party) {
      ...
      for (let share in meter.shares) {
        if (this.parties.find(p => p.uuid == share)) sum += meter.shares[share];
      }
