    ...
    <template #inner>
      <div class="gridContainer">
        ...
        <div id="readings" class="shadow">
          ...
          <p v-if="selectedMeter == null">
            Bitte zunächst einen Zähler auswählen.
          </p>
          <p v-else-if="getReadings().length == 0">
            Noch keine Ablesung für Zähler {{ selectedMeter.mid }} eingetragen.
          </p>
          <AddButton v-if="selectedMeter != null" v-on:click="addReading" />

          <p
            v-if="selectedMeter && selectedMeter.producesDirtyWater"
            id="abwasserNotice"
          >
            Eingetragene Zählerstände werden für Abwasserkosten berücksichtigt.
          </p>
        </div>
      </div>
    ...
    getMeters() {
      return this.meters.filter(
        m =>
          ...
          (this.selectedCategory.id == 3 && m.producesDirtyWater)
      );
    },
