...
export default {
  ...
  mounted() {
    ...
    let options = {
      ...
      title: {
        display: true,
        text:
          "Visualisierung der vorhandenen Daten für Zähler " +
          this.selectedMeter.mid +
          "."
      },
  ...
  methods: {
    getData() {
      let returns = [];
      let datapoints = [];
      let relevantReadings = this.readings
        .filter(reading => reading.m_uuid == this.selectedMeter.uuid)
            ...
            datapoints.push({
              ...
              y:
                ((next.value - current.value) /
                  (new Date(next.date) - new Date(current.date))) *
                (1000 * 60 * 60 * 24 * 365)
            });
          } else
            datapoints.push({
      ...
      if (this.displayCost) {
        let relevantContracts = this.contracts
          .filter(contract => contract.m_uuid == this.selectedMeter.uuid)
        ...
        for (let event of [...relevantContracts, ...relevantReadings]) {
          ...
          */
          let y =
            parseFloat(contract.costfix) +
            parseFloat(contract.costvar) * parseFloat(cons);
    ...
    getCurrentContract(date) {
      return this.contracts.find(
        contract =>
          new Date(contract.start) <= date && new Date(contract.end) >= date
      );
    },

    getCurrentYearlyConsumption(date) {
      let readings = this.readings.sort(
        (a, b) => new Date(a.date) > new Date(b.date)
      );
      let r1 = readings.find(reading => new Date(reading.date) >= date);
      let r2 = readings.find(
        reading => new Date(reading.date) >= date && reading != r1
      );

      if (r2) {
        let cons =
          ((r2.value - r1.value) / (new Date(r2.date) - new Date(r1.date))) *
          (365 * 24 * 60 * 60 * 1000);
        return cons;
      }
    }
